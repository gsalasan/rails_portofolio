...
class ReferencesControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create reference" do
    assert_difference("Reference.count") do
      post references_url, params: { reference: { email: @reference.email, name: @reference.name, position: @reference.position } }
    end
  ...
  test "should update reference" do
    patch reference_url(@reference), params: { reference: { email: @reference.email, name: @reference.name, position: @reference.position } }
